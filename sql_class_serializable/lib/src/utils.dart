...
final _columnKeyChecker = const TypeChecker.fromRuntime(ColumnKey);

DartObject columnKeyAnnotation(FieldElement element) =>
    _columnKeyChecker.firstAnnotationOfExact(element) ??
    (element.getter == null
        ? null
        : _columnKeyChecker.firstAnnotationOfExact(element.getter));

/// Returns `true` if [element] is annotated with [ColumnKey].
bool hasColumnKeyAnnotation(FieldElement element) =>
    columnKeyAnnotation(element) != null;
...
void throwUnsupported(FieldElement element, String message) =>
    throw InvalidGenerationSourceError(
        'Error with `@ColumnKey` on `${element.name}`. $message',
        element: element);
...
/// Return an instance of [TableSerializable] corresponding to a the provided
/// [reader].
TableSerializable _valueForAnnotation(ConstantReader reader) => TableSerializable(
...
/// Returns a [TableSerializable] with values from the [TableSerializable]
/// instance represented by [reader].
///
/// For fields that are not defined in [TableSerializable] or `null` in [reader],
/// use the values in [config].
TableSerializable mergeConfig(TableSerializable config, ConstantReader reader) {
...
/// If the enum value is annotated with [ColumnKey], then the `name` property is
