...
class _FieldSet implements Comparable<_FieldSet> {
  ...
  factory _FieldSet(FieldElement classField, FieldElement superField) {
    ...
    // Prefer the field that's annotated with `ColumnKey`, if any.
    // If not, use the class field.
    final fieldHasColumnKey =
        fields.firstWhere(hasColumnKeyAnnotation, orElse: () => fields.first);

    return _FieldSet._(fieldHasColumnKey, sortField);
  }
