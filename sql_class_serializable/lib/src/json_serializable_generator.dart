...
class TableSerializableGenerator
    extends GeneratorForAnnotation<TableSerializable> {
  ...
  final TableSerializable _config;

  TableSerializable get config => _config.withDefaults();

  /// Creates an instance of [TableSerializableGenerator].
  ///
  /// If [typeHelpers] is not provided, the built-in helpers are used:
  /// [BigIntHelper], [DateTimeHelper], [DurationHelper], [JsonHelper], and
  /// [UriHelper].
  const TableSerializableGenerator({
    TableSerializable config,
    List<TypeHelper> typeHelpers,
  })  : _config = config ?? TableSerializable.defaults,
  ...
  /// [BigIntHelper], [DateTimeHelper], [DurationHelper], [JsonHelper], and
  /// [UriHelper].
  factory TableSerializableGenerator.withDefaultHelpers(
  ...
  Iterable<String> generateForAnnotatedElement(
    ...
    if (element is! ClassElement) {
      ...
      throw InvalidGenerationSourceError('Generator cannot target `$name`.',
          todo: 'Remove the TableSerializable annotation from `$name`.',
          element: element);
    }
...
class _GeneratorHelper extends HelperCore with EncodeHelper, DecodeHelper {
  final TableSerializableGenerator _generator;
  ...
  Iterable<String> _generate() sync* {
    ...
    final accessibleFields = sortedFields.fold<Map<String, FieldElement>>(
      ...
      } else if (columnKeyFor(field).ignore) {
    ...
    accessibleFieldSet.fold(<String>{}, (Set<String> set, fe) {
      final columnKey = nameAccess(fe);
      if (!set.add(columnKey)) {
        throw InvalidGenerationSourceError(
            'More than one field has the JSON key `$columnKey`.',
            todo: 'Check the `ColumnKey` annotations on fields.',
            element: fe);
      }
      return set;
    });
