...
abstract class DecodeHelper implements HelperCore {
  ...
  void _writeChecks(int indent, TableSerializable classAnnotation,
    ...
    final requiredKeys =
        accessibleFields.where((fe) => columnKeyFor(fe).required).toList();
    ...
    final disallowNullKeys = accessibleFields
        .where((fe) => columnKeyFor(fe).disallowNullValue)
        .toList();
  ...
  String _deserializeForField(FieldElement field,
    ...
    final columnKeyName = safeNameAccess(field);
    ...
    try {
      if (config.checked) {
        ...
        if (!checkedProperty) {
          value = '\$checkedConvert(json, $columnKeyName, (v) => $value)';
        }
      } else {
        ...
        value = contextHelper
            .deserialize(targetType, 'json[$columnKeyName]')
            .toString();
      }
